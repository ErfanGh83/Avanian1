      ...
      >
        <div className="h-full flex flex-col items-end justify-end">
          {sender === "bot" ? (
            ...
            <div className="size-16 rounded-full flex items-center justify-center bg-blue-300"></div>
          )}
        </div>

        <div
          className={`max-w-[75%] h-fit min-h-[75%] bg-t/30 relative rounded-t-xl ${
            sender == "user" ? "bg-dark2 rounded-bl-xl" : "bg-avanianBlue rounded-br-xl"
          }`}
        >
          <div
            className={`absolute ${
              sender == "bot" ? "left-[-4px] top-[45px]" : "right-[-4px]"
            } bottom-0`}
          >
            <svg
              width="10"
